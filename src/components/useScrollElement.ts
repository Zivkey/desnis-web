...
export function useScrollElement(
  ...
  // Enter: 0→0.08, Exit: 0.92→1.0 — these sit right next to each other in global scroll
  const mEnter = 0;
  const mFull = 0.08;
  const mExitStart = 0.92;
  const mExitEnd = 1.0;

  const opacity = useTransform(
    progress,
    isMobile ? [mEnter, mFull, mExitStart, mExitEnd] : [enter, full, exitStart, exitEnd],
    [0, 1, 1, 0]
  );
  ...
  const y = useTransform(
    progress,
    isMobile ? [mEnter, mFull, mExitStart, mExitEnd] : [enter, full, exitStart, exitEnd],
    isMobile ? [15, 0, 0, -15] : [enterY, 0, 0, exitY]
  );
